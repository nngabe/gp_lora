...
where $f(\mathbf{x},\mathbf{z})$ is a deep GP conditioned on a context embedding $\mathbf{z}$ generated by our finetuned LLaMa model. The parameter $\omega$ defines the VaR as the minimum upper bound required such that $\mathbb{E}[f(\mathbf{x},\mathbf{z})]$ is less than the specified risk level.
